...
char *fpx_substr_replace(const char *haystack, const char *needle,
  ...
  const int returnedHaystackLen =
      fpx_getstringlength(haystack) + replacementLen - needleLen;
  ...
  /*
   * add null-byte (\0) to the end of the char array, to denote the end of the
   * string (this null-byte was accounted for when allocating memory for the
   * result string)
   */
...
char *fpx_string_to_upper(const char *input, int doReturn) {
   ...
   * (doReturn != 0) Returns new heap-allocated, null-terminated char[] when
   * finished
   *  ||
   * (doReturn == 0) Modifies input string and returns NULL (will not allocate
   * heap)
   */
  const int inputLength = fpx_getstringlength(input);
  char *inputCopy =
      (doReturn) ? (char *)malloc(inputLength + 1) : (char *)input;

  for (int i = 0; i < inputLength; i++) {
    inputCopy[i] =
        (input[i] > 96 && input[i] < 123) ? (input[i] - 32) : input[i];
  }
...
char *fpx_string_to_lower(const char *input, int doReturn) {
  ...
  for (int i = 0; i < inputLength; i++) {
    inputCopy[i] =
        (input[i] > 64 && input[i] < 91) ? (input[i] + 32) : input[i];
  }
