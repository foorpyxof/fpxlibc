...
int main() {
  ...
  sll.PrependNode(
      new LinkedListNode(4)); // add 4 to the start of the linked-list
  sll.PrependNode(
      new LinkedListNode(9)); // add 9 to the start of the linked-list
  ...
  sll.InsertNode(new LinkedListNode(21),
                 1); // insert new node with value 21 between index 0 and 1
  sll.InsertNode(node1PTR,
                 2); // insert pre-made node (123) between index 1 and 2
  ...
  std::cout << "v2 empty? (expect false/0) " << v2.IsEmpty()
            << std::endl; // expect: false/0
  v2.PopBack();
  std::cout << "v2 empty? (expect true/1) " << v2.IsEmpty()
            << std::endl; // expect: true/1
