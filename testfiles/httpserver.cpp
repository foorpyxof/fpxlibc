...
void WebSocketCallback(HttpServer::websocket_client_t* client, uint16_t metadata, uint64_t len,
  uint32_t mask_key, uint8_t* data) {
  printf("MESSAGE: %s\n", data);
...
void UserAgentCallback(HttpServer::http_request_t* req, HttpServer::http_response_t* res) {
  ...
  if (userAgentIndex > -1) {
    char tempBuf[256];
    snprintf(tempBuf,
      strcspn(&req->Headers[userAgentIndex], "\r\n") + 1,
      "%s",
...
int main() {
  ...
  // callback. ^^^ make sure to add all the required WebSocket headers in the response before
