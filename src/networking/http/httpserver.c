...
#define SET_HTTPRESPONSE_PRESET(srvptr, res, code, reason_text, body, body_len)              \
  {                                                                                          \
    if (0 == fpx_httpresponse_set_version(&res, STR(HTTP_VERSION)) && (res.status = code) && \
...
int fpx_httpserver_listen(fpx_httpserver_t* srvptr, const char* ip, const uint16_t port) {
  ...
  // socket things
  {
    ...
    {
      int one = 1;
      setsockopt(listen_socket, SOL_SOCKET, SO_REUSEADDR, (void*)&one, sizeof(one));
    }
 ...
 * - return -4 if the given `out_len` is too little. `copied_count` will then be set to the required
 * length
 */
static int _sanitize_headers(const char* in, char* out, size_t out_len, size_t* copied_count) {
...
// to ignore SIGPIPE when writing to a disconnected client
  ...
  // body
  if (resptr->content.body_len > 0 && (NULL == reqptr || FALSE == (reqptr->method & HTTP_HEAD))) {
...
static int _ws_parse_request(int fd, fpx_websocketframe_t* output) {
  ...
  } while (0 < left_to_read && -1 != (amount_read = recv(fd, (char*)readbuf, recv_size, 0)));
